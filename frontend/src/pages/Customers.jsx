...
export default function Customers() {
  ...
  const fetchCustomers = async () => {
    try {
      setLoading(true)
      const data = await customerService.list({ shop_id: shop.id })
      setCustomers(data)
    } catch (err) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          filteredCustomers.map(customer => (
            <div key={customer.id} className="glass rounded-3xl p-6 hover:bg-white/[0.03] transition-all duration-300 group">
